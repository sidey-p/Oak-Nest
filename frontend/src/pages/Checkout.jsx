...
const Checkout = () => {
  ...
  useEffect(() => {
    if (!loading && cartCount === 0 && !placedOrder) navigate('/cart');
  }, [cartCount, loading, placedOrder]);
    ...
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
        ...
        {STEPS.map((s, i) => (
          <li key={s} className="flex flex-1 items-center last:flex-none">
            <div className="flex flex-col items-center">
              <span className={`grid h-9 w-9 place-items-center rounded-full text-sm font-bold ${i < step ? 'bg-green-600 text-white' : i === step ? 'bg-brand-800 text-white' : 'bg-brand-200 text-brand-500'}`}>
                {i < step ? '✓' : i + 1}
              </span>
              <span className="mt-1.5 text-[11px] font-semibold uppercase tracking-wide text-brand-600">{s}</span>
            </div>
            {i < STEPS.length - 1 && <div className={`mx-2 h-0.5 flex-1 rounded ${i < step ? 'bg-green-600' : 'bg-brand-200'}`} />}
          </li>
        ))}
      </ol>
      ...
      {step === 0 && (
        <section className="mt-8 space-y-4">
          {addresses.map((a) => (
            <label key={a.id} className={`flex cursor-pointer gap-4 rounded-2xl border-2 p-5 transition ${selectedAddress === a.id ? 'border-accent-600 bg-white' : 'border-brand-200 bg-white hover:border-brand-400'}`}>
          ...
          <button disabled={!selectedAddress} onClick={() => setStep(1)}
            className="w-full rounded-xl bg-brand-800 py-3.5 text-sm font-bold text-white hover:bg-brand-700 disabled:opacity-40">
      ...
      {/* STEP 1 — SUMMARY */}
      {step === 1 && (
        <section className="mt-8 rounded-2xl border border-brand-200 bg-white p-6">
          ...
          <div className="mt-6 flex gap-3">
            <button onClick={() => setStep(0)} className="rounded-xl border border-brand-300 px-6 py-3 text-sm font-semibold">← Back</button>
            <button onClick={() => setStep(2)} className="flex-1 rounded-xl bg-brand-800 py-3 text-sm font-bold text-white hover:bg-brand-700">Continue to Payment</button>
          </div>
        </section>
      )}
      ...
      {step === 2 && (
        <section className="mt-8 space-y-4">
          <div className={`cursor-pointer rounded-2xl border-2 p-5 transition ${paymentMethod === 'cash_on_delivery' ? 'border-accent-600 bg-white' : 'border-brand-200 bg-white'}`} onClick={() => setPaymentMethod('cash_on_delivery')}>
          ...
          <div className={`cursor-pointer rounded-2xl border-2 p-5 transition ${paymentMethod === 'local_test_payment' ? 'border-accent-600 bg-white' : 'border-brand-200 bg-white'}`} onClick={() => setPaymentMethod('local_test_payment')}>
            <div className="flex items-center gap-3">
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="rounded-xl border border-brand-300 px-6 py-3 text-sm font-semibold">← Back</button>
            <button onClick={placeOrder} disabled={busy}
              className="flex-1 rounded-xl bg-green-700 py-3 text-sm font-bold text-white hover:bg-green-600 disabled:opacity-50">
      ...
      {step === 3 && placedOrder && (
        <section className="mt-10 text-center">
          ...
          <h2 className="mt-6 font-serif text-3xl font-bold text-brand-900">Order Confirmed!</h2>
          <p className="mt-2 text-brand-600">Thank you for shopping with Furnishing Essentials.</p>
          <div className="mx-auto mt-6 max-w-md rounded-2xl border border-brand-200 bg-white p-6 text-sm">
          ...
          <div className="mt-8 flex justify-center gap-3">
            <Link to="/orders" className="rounded-xl bg-brand-800 px-8 py-3 text-sm font-bold text-white hover:bg-brand-700">View My Orders</Link>
            <Link to={`/orders/${placedOrder.id}/tracking`} className="rounded-xl border border-brand-300 px-8 py-3 text-sm font-bold text-brand-800 hover:bg-white">Track Order</Link>
          </div>
        </section>
      )}
    </div>
