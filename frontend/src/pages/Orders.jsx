    ...
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      {orders.length === 0 ? (
        ...
        <div className="mt-8 space-y-4">
          {orders.map((o) => (
            <div key={o.id} className="rounded-2xl border border-brand-200 bg-white p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm">
                  ...
                  <span className={`font-semibold ${o.payment_status === 'paid' ? 'text-green-700' : 'text-amber-700'}`}>{o.payment_status}</span>
                </div>
                <div className="flex gap-2">
                  <Link to={`/orders/${o.id}`} className="rounded-lg border border-brand-300 px-4 py-2 text-xs font-semibold hover:bg-brand-50">View Details</Link>
                  <Link to={`/orders/${o.id}/tracking`} className="rounded-lg bg-brand-800 px-4 py-2 text-xs font-semibold text-white hover:bg-brand-700">Track</Link>
                </div>
              </div>
            </div>
          ))}
