...
const Cart = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <h1 className="font-serif text-3xl font-bold text-brand-900">Shopping Cart</h1>

      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="space-y-4">
          {cart.items.map((item) => (
            <div key={item.id} className="flex gap-4 rounded-2xl border border-brand-200 bg-white p-4">
              ...
              <div className="flex flex-1 flex-col">
                ...
                <div className="mt-auto flex items-center justify-between pt-3">
                  <div className="flex items-center rounded-lg border border-brand-300">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-3 py-1.5 text-brand-600 hover:text-brand-900">−</button>
          ...
          <Link to="/products" className="inline-block text-sm font-semibold text-accent-600 hover:underline">← Continue shopping</Link>
        </div>

        <aside className="h-fit rounded-2xl border border-brand-200 bg-white p-6 lg:sticky lg:top-24">
          ...
          <form onSubmit={applyCoupon} className="mt-4 flex gap-2">
            ...
            <button className="rounded-lg bg-brand-700 px-4 text-sm font-semibold text-white hover:bg-brand-600">Apply</button>
          </form>
          {couponError && <p className="mt-2 text-xs text-red-600">{couponError}</p>}
          {coupon && <p className="mt-2 text-xs text-green-700">Coupon applied ✓</p>}
          ...
          <button onClick={() => navigate('/checkout', { state: { coupon } })} disabled={loading}
            className="mt-5 w-full rounded-xl bg-brand-800 py-3.5 text-sm font-bold text-white hover:bg-brand-700 disabled:opacity-50">
            Proceed to Checkout
          </button>
        </aside>
      </div>
