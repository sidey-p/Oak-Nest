...
export const Button = ({ as: As = 'button', variant = 'primary', size = 'md', className = '', children, ...props }) => {
  ...
  return (
    <As
      className={`btn-shine inline-flex items-center justify-center gap-2 rounded-full font-semibold transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] shadow-sm hover:shadow-md active:scale-[0.96] disabled:cursor-not-allowed disabled:opacity-40 ${variants[variant]} ${sizes[size]} ${className}`}
      {...props}
    >
      {children}
    </As>
